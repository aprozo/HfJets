    ...
    StTagD0MCEvents::StTagD0MCEvents(const char *name, StPicoDstMaker *picoMaker, const char *outName = "") : StJetFrameworkPicoBase(name) // StMaker(name),
{
  ...
  fTowerBias = 0.0;
  fJetRad = 0.4;
  fEventZVtxMinCut = -60.0;
  fEventZVtxMaxCut = 60.0;
...
{
  if (fPrintLevel)
    cout << "Called StTagD0MCEvents" << endl;
  ...
  if (fDebugLevel == StJetFrameworkPicoBase::kDebugEmcTrigger)
    cout << "EventTriggers: ";
  ...
  if (doppAnalysis)
    fRunForMB = (fHaveMBevent) ? kTRUE : kFALSE;
  if (!doppAnalysis)
    fRunForMB = (fHaveMB5event || fHaveMB30event) ? kTRUE : kFALSE;

  int arrMB5[] = {520001, 520011, 520021, 520031, 520041, 520051, 570002, 570001};
  int arrHT1[] = {550201, 560201, 540201};
  int arrHT2[] = {540203, 530202};
  int arrHT3[] = {520203, 530213, 570203};
  ...
  if (!doppAnalysis)
  {
    ...
    for (int i = 0; i < sizeof(arrHT1) / sizeof(*arrHT1); i++)
    {
      if (mPicoEvent->isTrigger(arrHT1[i]))
        matchHT1 = kTRUE;
      if (matchHT1)
        break;
    }

    for (int i = 0; i < sizeof(arrHT2) / sizeof(*arrHT2); i++)
    {
      if (mPicoEvent->isTrigger(arrHT2[i]))
        matchHT2 = kTRUE;
      if (matchHT2)
        break;
    }

    for (int i = 0; i < sizeof(arrHT3) / sizeof(*arrHT3); i++)
    {
      if (mPicoEvent->isTrigger(arrHT3[i]))
        matchHT3 = kTRUE;
      if (matchHT3)
        break;
    }

    if (matchHT1 || matchHT2 || matchHT3)
      numberofevents[6]++;

    if (matchHT2 || matchHT3)
      numberofevents[7]++;
...
map<int, vector<int>> StTagD0MCEvents::GetMCTrackListForVertex()
{
...
void StTagD0MCEvents::GetD0TracksPair()
{
  ...
  for (unsigned short track1 = 0; track1 < nmctracks; track1++)
  {
    ...
    if (mctrk1->geantId() == 37 || mctrk1->geantId() == 38)
    {
      ...
      if (mctrk1->p().Perp() < 1 || mctrk1->p().Perp() > 10)
        continue;
      if (mctrk1->p().PseudoRapidity() < -1 || mctrk1->p().PseudoRapidity() > 1)
        continue;
  ...
  for (unsigned short track1 = 0; track1 < nmctracks; track1++)
  {
    ...
    for (unsigned short track2 = 0; track2 < nmctracks; track2++)
    {
      ...
      if (!((firsttrack == 8 && secondtrack == 12) || (firsttrack == 9 && secondtrack == 11)))
        continue; // Makes sure it's always pions + kaons (not the other way around)

      if (!IsD0MCTrack(track1) || !IsD0MCTrack(track2))
        continue; // If both tracks are not from D0s, then discard

      int idvxstart1 = mctrk1->idVtxStart();
      int idvxstart2 = mctrk2->idVtxStart();

      if (idvxstart1 != idvxstart2)
        continue; // If both tracks are not from the same D0, then discard
...
void StTagD0MCEvents::MCTracksToDiscard()
{ // This is for MC tracks
  ...
  if (fPrintLevel)
    cout << "Dropped Track Array Size = " << fDroppedMCTracks.size() << endl;
}
...
void StTagD0MCEvents::FindTracksToDrop()
{
  ...
  for (int d0 = 0; d0 < fd0RecoTrackIndices.size(); d0++)
  {
    ...
    for (unsigned short iTracks = 0; iTracks < ntracks; iTracks++)
    {
      ...
      if (iTracks == fd0RecoTrackIndices[d0][0] || iTracks == fd0RecoTrackIndices[d0][1])
        continue;
...
void StTagD0MCEvents::ReplacePoorlySmearedTracks()
{
  ...
  for (unsigned short iTracks = 0; iTracks < ntracks; iTracks++)
  {
    ...
    if (fReplaceBadTracks && trackeligibleforreplacement)
    {
      ...
      if (pid == 8 || pid == 9)
        fastsimsmearing = fPionMomResolution->Eval(mctrk->p().Pt()); // Pion
      else if (pid == 11 || pid == 12)
        fastsimsmearing = fKaonMomResolution->Eval(mctrk->p().Pt()); // Kaon
      else if (pid == 15 || pid == 14)
        fastsimsmearing = fProtonMomResolution->Eval(mctrk->p().Pt()); // Proton
      else
        fastsimsmearing = fPionMomResolution->Eval(mctrk->p().Pt()); // Catch all: pions
...
Bool_t StTagD0MCEvents::IsD0MCTrack(int trackid)
{
  ...
  for (unsigned short track1 = 0; track1 < mPicoDst->numberOfMcTracks(); track1++)
  {
    ...
    if (mctrk1->geantId() == 37 || mctrk1->geantId() == 38)
    {
      ...
      if (idvxstop < 0)
        return d0Track; // Need this check. This should never happen for a D0
...
Bool_t StTagD0MCEvents::IsTrackADescendantOfD0Daughters(int trackid, int D0num)
{ // This is for reco tracks (Needs to be called after getting the list of discarded tracks from MC)
  StPicoTrack *trk = static_cast<StPicoTrack *>(mPicoDst->track(trackid));
  int mctrkid = trk->idTruth() - 1;

  if (std::find(fDroppedMCTracks[D0num].begin(), fDroppedMCTracks[D0num].end(), mctrkid) != fDroppedMCTracks[D0num].end())
    return kTRUE;
...
Bool_t StTagD0MCEvents::KeepTrack(int trackid) // This is for Reco Tracks
{
  ...
  if (abs(trk->nSigmaPion()) < 2. && abs(trk->nSigmaKaon()) > 2. && abs(trk->nSigmaProton()) > 2.)
    particleid = 1 * trk->charge();
  else if (abs(trk->nSigmaPion()) > 2. && abs(trk->nSigmaKaon()) < 2. && abs(trk->nSigmaProton()) > 2.)
    particleid = 2 * trk->charge();
  else if (abs(trk->nSigmaPion()) > 2. && abs(trk->nSigmaKaon()) > 2. && abs(trk->nSigmaProton()) < 2.)
    particleid = 3 * trk->charge();
  ...
  if (particleid == 1 || particleid == -1)
    keeptrack = (rando > fPionWeight->Eval(pt)) ? kFALSE : kTRUE;
  else if (particleid == 2 || particleid == -2)
    keeptrack = (rando > fKaonWeight->Eval(pt)) ? kFALSE : kTRUE;
  else if (particleid == 3)
    keeptrack = (rando > fProtonWeight->Eval(pt)) ? kFALSE : kTRUE;
  else if (particleid == -3)
    keeptrack = (rando > fAProtonWeight->Eval(pt)) ? kFALSE : kTRUE;
