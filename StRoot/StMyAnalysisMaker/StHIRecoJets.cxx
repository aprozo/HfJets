...
void StHIRecoJets::FillJetBGBranch(int iteration)
{
  ...
  for (UInt_t ijet = 0, jetCount = 0; ijet < jets_incl.size(); ++ijet)
  {
    ...
    if (fPrintLevel){
      ...
      if (doConstituentSubtr) cout << GetName() << "\t" << "D0 User Index Before And After:" << d0UserIndex << "\t" << constituents[d0index].user_index() << "\t" << fRecoTracks[iteration][d0UserIndex-10000].Pt() << "\t" << constituents[d0index].pt() << "\t" << jet->Pt() << endl;
    }
    // set label
    jet->SetLabel(ij);
