...
Int_t StCentMaker::Make()
{
  ...
  if (!doppAnalysis)
  {
    ...
    if (doUseBBCCoincidenceRate)
    {
      grefmultCorr->initEvent(kgrefMult, zVtx, fBBCCoincidenceRate);
    } // default
    else
    {
      grefmultCorr->initEvent(kgrefMult, zVtx, fZDCCoincidenceRate);
    }
    ...
    if (doUseBBCCoincidenceRate)
    {
      krefCorr2 = grefmultCorr->getRefMultCorr(kgrefMult, zVtx, fBBCCoincidenceRate, 2);
    }
    else
    {
      krefCorr2 = grefmultCorr->getRefMultCorr(kgrefMult, zVtx, fZDCCoincidenceRate, 2);
    }
  ...
  // debug
  if (fDebugLevel == kDebugCentrality)
  {
    if (centbin > 15)
      cout << "centbin = " << centbin << "  mult = " << krefCorr2 << "   kCentralityScaled = " << kCentralityScaled << "  cent16 = " << kcent16 << endl;
  }
                                               ...
                                               const Double_t zdcCoincidenceRate, const UInt_t flag)
{
  ...
  double refCorr = -99.;
  if (flag == 0)
    refCorr = grefmultCorr->getRefMultCorr(RefMult, z, zdcCoincidenceRate, flag);
  if (flag == 1)
    refCorr = grefmultCorr->getRefMultCorr(RefMult, z, zdcCoincidenceRate, flag);
  if (flag == 2)
    refCorr = grefmultCorr->getRefMultCorr(RefMult, z, zdcCoincidenceRate, flag);
