...
void MCResponse(const Char_t *inputFile = "test.list", const Char_t *outputFile = "test.root", Int_t nEv = 1, const Char_t *fOutJobappend = "")
{
  ...
  if ((RunYear == mRun14) && doTEST)
  {
    ...
    inputFile = "/gpfs01/star/pwg/prozorov/HFjets/analysis/test2014.list";
  }
  // if((RunYear == mRun14) && doTEST) inputFile = "SampleFileListVertex.list";
  if ((RunYear == mRun16) && doTEST)
    inputFile = "/gpfs01/star/pwg/prozorov/HFjets/analysis/test2016.list";
  ...
  if (RunYear == mRun14)
    CentralityDefinition == StJetFrameworkPicoBase::kgrefmult_P18ih_VpdMB30_AllLumi; // (NEW - from Nick Aug22, 2019: all lumi) this will be default
  if (RunYear == mRun14 && MCSim)
    CentralityDefinition == StJetFrameworkPicoBase::kgrefmult_P18ih_VpdMB30; // no centrality defintion for pp, just set one
  ...
  for (int centbin = 2; centbin < 3; centbin++)
  {
    ...
    D0MCTaggerSmear[centbin]->SetPrintLevel(1);
